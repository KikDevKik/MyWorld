...
const LinksOverlayV2 = forwardRef<LinksOverlayHandle, {
    ...
    // ⚡ Bolt Optimization: Pre-compute lookups AND link list to avoid O(N^2) in render loop
    // This decouples "link discovery" (dependent on nodes) from "link styling" (dependent on hover)
    const links = useMemo(() => {
        // 1. Build Maps (O(N))
        ...
        const computedLinks: RenderableLink[] = [];
        ...
        nodes.forEach(node => {
            ...
            node.relations.forEach((rel, idx) => {
                ...
                computedLinks.push({
                    ...
                    color: relColor,
                    label: labelText,
                    fullLabel: `${rel.relation}: ${rel.context || 'Sin contexto'}`,
