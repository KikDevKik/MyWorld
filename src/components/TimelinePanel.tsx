...
// ⚡ Bolt Optimization: Memoized Event Item to prevent re-renders on input typing
...
const TimelineEventItem = memo(({ event, index, onFileSelect, onApprove, onDiscard }: TimelineEventItemProps) => {
    const isLeft = index % 2 === 0;
    const isSuggested = event.status === 'suggested';

    return (
        <li className={`flex items-center w-full ${isLeft ? 'flex-row' : 'flex-row-reverse'}`}>
            {/* Card */}
...
const TimelinePanel: React.FC<TimelinePanelProps> = ({ onClose, userId, onFileSelect, currentFileId, accessToken, isSecurityReady = false }) => {
    ...
    */
    ...
    const handleDiscard = useCallback(async (eventId: string) => {
        if (!userId) return;
        const db = getFirestore();
        const eventRef = doc(db, 'TDB_Timeline', userId, 'events', eventId);
        try {
